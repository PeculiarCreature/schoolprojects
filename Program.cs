...
Game 100: 12 red, 9 green; 12 red; 9 red, 3 green; 8 red, 4 blue, 4 green; 8 blue, 11 red, 2 green";
            ...
            {
                string[] xxx = lines[a].Split(":");
                string[] sets = xxx[1].Split(";");
                foreach (var set in sets)
                {

                    string[] cubes = sets[b].Split(",");
                    ...
                    foreach (var cube in cubes)
                    {
                        ...
                        string[] color = cubes[c].Split(" ");

                        c++;
                        Console.WriteLine(color[1] + ":" + color[2]);

                        if (color[2] == "green")
                        {
                            if (int.Parse(color[1]) > green)
                            {
                                games += a + 1;
